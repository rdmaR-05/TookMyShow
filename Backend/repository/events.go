...
func (repo *EventRepository) GetSeatPrice(ctx context.Context, instanceID uuid.UUID, seatID uuid.UUID) (float64, error) {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()
	...
	query := `
		SELECT pt.price
		FROM event_seat es
		JOIN price_tier pt ON es.event_zone_id = pt.event_zone_id
		WHERE es.event_instance_id = $1 AND es.seat_id = $2
	`
	var price float64
	err := repo.Conn.QueryRow(ctx, query, instanceID, seatID).Scan(&price)
	return price, err
}
